...
const JobCard = ({ job }) => {
  console.log(job)
  const { id, company, title, isFeatured, location, salary, job_tags: jobTags } = job
  const fromattedTimeSinceAdded = '666 days ago'
  return (
    <div className='rounded-md bg-indigo-700 overflow-hidden shadow-md w-full'>
        ...
        <div className='flex items-center'>
          ...
          <div className='ml-2'>
            <div className='flex items-baseline'>
              <div className='text-indigo-700 text-sm'>{company.name}</div>
              <div>
                {isFeatured && <div className='px-2 ml-1 bg-indigo-700 text-gray-100 text-xs font-semibold rounded-full uppercase'>Featured</div>}
              </div>
            </div>
            <div className='text-md font-bold font-gray-900 mt-1'>{title}</div>
            <div className='flex text-xs text-gray-600'>
              <span className='truncate'>{fromattedTimeSinceAdded}</span>
            ...
            <div className='flex flex-wrap mt-1 lg:hidden'>
              {jobTags.map(tag =>
                <div key={tag.tag.id} className='flex items-center mt-2 mr-1 px-2 bg-indigo-200 text-indigo-600 uppercase rounded-md text-xs tracking-wide font-semibold'>
                  {tag.tag.name}
                </div>)}
            </div>
          </div>
